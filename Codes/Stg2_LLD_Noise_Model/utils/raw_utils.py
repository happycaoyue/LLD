...
# elif camera_code == "G4":
#     bayer_pattern = "BGGR"
def flip_bayer(image, bayer_pattern, print_flag = False):
...
# im[1:h:2, 0:w:2, :] g
# im[1:h:2, 1:w:2, :] b
def unpack_raw_channels(raw4ch):
    ...
    bayer[1::2, 0::2] = raw4ch[:, :, 2]
    bayer[1::2, 1::2] = raw4ch[:, :, 3]
    return bayer
...
def simple_raw_to_rgb(rggb_channels_stack, black=0, wb=np.array([[1], [1], [1]])):
    ...
    # H, W, _ = rggb_channels_stack.shape
    # C = 3
    # channels_rgb = np.zeros((H, W, C), dtype=np.float32)
    # channels_rgb = raw_rggb_stack[:, :, :3]
    channels_rgb = rggb_channels_stack[:, :, :3]
    # print(channels_rgb.shape)
...
def simple_raw_rggb_stack_to_rgb(raw_rggb_stack):
    ...
    ###
    channels_rgb[:, :, 2] = raw_rggb_stack[:, :, 0]
    channels_rgb[:, :, 0] = raw_rggb_stack[:, :, 3]
    return channels_rgb
